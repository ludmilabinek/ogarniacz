...
_PRD's `## Open Questions` resolved during `/10x-shape` and `/10x-prd`. Per-slice unknowns live in each slice's `Unknowns` field. Cross-cutting questions surfaced post-ship are recorded below._

### Q1: Wedge accuracy gap on real announcements (open since 2026-06-12)

- **Finding:** S-05 shipped, but the LLM extraction regression harness (test-plan §3 Phase 1) measured first-extraction accuracy of **1/9 ≈ 11.1%** on the seed batch of real announcements and projected **~62.5%** after the `llm-prompt-year-resolution` prompt fix. PRD's success criterion is **≥80%** on representative real announcements (FR-004, FR-005, NFR).
- **Sources:** `context/archive/2026-06-12-llm-fixture-set-expansion/triage.md` (per-fixture detail + post-prompt-fix projection); `context/archive/2026-06-12-llm-prompt-year-resolution/` (prompt fix that addressed the dominant error class — 15 of 22 documented divergences = 68% — by resolving missing-year dates).
- **Why this is a roadmap-level question, not a slice unknown:** the S-05 Risk section pre-committed to surfacing accuracy below 80% as "a PRD-level finding to surface — not a slice that fails silently". S-05 is archived; without this entry the gap is invisible to anyone reading the roadmap.
- **Next-action options (not yet chosen):**
  - **(a) Iterate on the prompt + grow the fixture set** to separate systematic divergences (addressable by prompt) from random model error (addressable only by swapping model). Cheap; uses the harness that already exists.
  - **(b) Swap the OpenRouter vision model** (currently default per F-01). The harness's recorded-mock variant makes model swaps cheap to re-baseline.
  - **(c) Revise PRD §Success Criteria** with the measurement and a stated rationale (e.g. accept lower accuracy if curator review is cheap enough, or scope down the announcement classes targeted in MVP). Heaviest option; only after (a) and (b) are exhausted.
- **Owner:** curator. **Block:** does not block existing slices; gates the wedge hypothesis claim.
...
- **S-05: parent uploads an image (camera capture or screenshot) of a kindergarten announcement; within 60 seconds they see a list of one or more AI-proposed events with editable fields; they can accept or reject each individually; accepted events flow into the personal view (S-02's pipeline) and into the parent's subscribed calendar via the iCalendar feed (S-04's pipeline). Unreadable images surface an actionable error with a retry / manual-entry path.** — Archived 2026-06-21 → `context/archive/2026-06-16-image-extraction-and-review-acceptance/`. **Wedge measurement (open):** first-extraction accuracy on the seed batch was 1/9 ≈ 11.1% (`context/archive/2026-06-12-llm-fixture-set-expansion/triage.md`), projected ~62.5% after the `llm-prompt-year-resolution` prompt fix — below PRD's ≥80% target. The S-05 Risk section pre-committed to surfacing this finding at roadmap level rather than letting the wedge hypothesis silently fail; see Open Roadmap Questions Q1. Lesson: an archived measurement that contradicts a "done" slice's success criterion must be promoted out of the change folder — otherwise the roadmap reads "shipped + validated" when only "shipped" is true.
